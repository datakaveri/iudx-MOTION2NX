#ifndef FUNCTIONS_CNN_NN_1_H
#define FUNCTIONS_CNN_NN_1_H
...
#include "GlobalVar_3PC_CNN_NN_1.h"
  ...
  if (choice == 1 && layer_type == 1) {
    ...
    if (layer_id == 1) {
      ...
      if (convToMatMulFlag == true) {
        ...
        if (Output_Public_Shares.size() != rows * col || Output_Private_Shares.size() != rows * col) {
          std::cerr << "Dimensions do not match." << std::endl;
          std::cout << "Output_Public_Shares.size(): " << Output_Public_Shares.size() << " " << rows * col << std::endl;
          std::cout << "Output_Private_Shares.size(): " << Output_Private_Shares.size() << " " << rows * col << std::endl;
          return EXIT_FAILURE;
        }
...
int MatrixMultAddition(std::vector<std::uint8_t>& message_w, std::vector<std::uint8_t>& message_i) {
  ...
  try {
    comm_layer->send_message(other_party, mes1);
  }
  catch (std::runtime_error& e) {
...
void operationsConvolutionOperation() {  
  //prod1=Delw * delx1
  //std::vector<std::uint64_t>prod1=multiplicate(wpublic,xsecret);
...
int ConvolutionReluLayer(std::vector<std::uint8_t> weightSharesMessage, std::vector<std::uint8_t> imageSharesMessage, int layer_id) {
  ...
  std::cerr << "Layer " << layer_id << ": Convolution layer done." << std::endl;
  ...
  if ((reluRows != privateReluRows) || (reluCols != privateReluCols)) {
    std::cerr << "Dimensions do not match." << std::endl;
    std::cerr << "Dimensions of the convolution public shares: " << reluRows << " " << reluCols << std::endl;
    std::cerr << "Dimensions of the convolution private shares: " << privateReluRows << " " << privateReluCols << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::cerr << "Layer " << layer_id << ": ReLU operation done." << std::endl;
...
int MatrixMultiplicationLayer(std::vector<std::uint8_t>& weightSharesMessage, std::vector<std::uint8_t>& imageSharesMessage, int layer_id) {
  ...
  std::cerr << "Layer " << layer_id << ": Matrix addition and multiplication done." << std::endl;
...
int MatrixMultiplicationReluLayer(std::vector<std::uint8_t>& weightSharesMessage, std::vector<std::uint8_t>& imageSharesMessage, int layer_id) {
  Output_Public_Shares.clear();
  Output_Private_Shares.clear();
  std::cout << "Entered MatrixMultiplicationReluLayer." << std::endl;
  HelperNodeSyncFunction();
  ...
  std::cerr << "Layer " << layer_id << ": ReLU done." << std::endl;
  ResetFlags(0);
  return EXIT_SUCCESS;
}
