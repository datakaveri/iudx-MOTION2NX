#ifndef FUNCTIONS_CNN_NN_DEBUG_0_H
#define FUNCTIONS_CNN_NN_DEBUG_0_H
...
#include "GlobalVar_3PC_CNN_NN_debug_0.h"
...
void ConvertMessageIntoVector(std::vector<uint8_t>& msg, std::vector<uint64_t>& vec) {
  ...
  std::cout << "##### Exiting ConvertMessageIntoVector. #####\n\n"; 
}
...
void read_shares(int choice, std::vector<uint8_t>&message, int layer_id, int layer_type, const Options& options, bool ConvToMatMulFlag) { 
  ...
  if (choice == 1 && layer_type == 1) { 
    ...
    try {
      ...
      std::cerr << conv_kernels << " " << conv_channels << " " << conv_rows << " " << conv_cols << std::endl;
      for (int i = 0; i < 4; i++) {
        std::uint64_t temp;
        file >> temp;
        pads[i] = temp;
  ...
  // Reading the image shares from the respective filepath
    ...
    std::string fullpath = options.current_path;
    if (layer_id == 1) {
      std::cerr << "Entered this block." << std::endl;
      ...
      if (output_chnls != image_channels || output_rows != image_rows || output_columns != image_cols || Output_Public_Shares.size() != image_channels * image_rows * image_cols + 3 || Output_Private_Shares.size() != image_channels * image_rows * image_cols + 3) {
        std::cerr << "Dimensions of shares do not match." << std::endl;
        std::cerr << Output_Public_Shares.size() << std::endl;
        std::cerr << Output_Private_Shares.size() << std::endl;
...
int ConvolutionReluLayer(std::vector<std::uint8_t>& weightSharesMessage, std::vector<std::uint8_t>& imageSharesMessage, int layer_id) {
  // Step 1: Helpernode sync.
  HelperNodeSyncFunction();

  // Step 2: Perform convolution operation, store shares in A_Conv_Public_Shares, A_Conv_Private_Shares.
  ConvolutionOperation(weightSharesMessage, imageSharesMessage);

  // Step 3: Prepare for ReLU.
  try {
  ...
  if (rowsABYShares * colsABYShares != output_chnls * output_rows * output_columns) {
    std::cerr << "Dimensions do not match." << std::endl;
    return EXIT_FAILURE;
  }
...
std::vector<std::uint64_t> convolution(std::vector<std::uint64_t> input, std::vector<std::uint64_t> weights, std::uint64_t kernels, std::uint64_t channels,
  ...
  std::cerr << img_rows << " " << img_cols << " " << channels << " " << std::endl; 
...
void operations() {       
  ...
  std::cerr << xpublic.size() << " " << wpublic.size() << std::endl;
