#include "Functions_3PC_CNN_NN_debug_0.h"
...
int main(int argc, char* argv[]) {
  ...
  for (int layer_id = 1; layer_id <= numberOfLayers; layer_id++) {
    ...
    if (layer_type == 1 && layer_id == 1) {
      read_shares(1, weightSharesMessage, layer_id, layer_type, options.value(), ConvToMatMulFlag);
      read_shares(2, imageSharesMessage, layer_id, layer_type, options.value(), ConvToMatMulFlag);
  ...
  std::size_t outputChannels = Output_Public_Shares[0];
  std::size_t outputRows = Output_Public_Shares[1];
  std::size_t outputColumns = Output_Public_Shares[2];

  std::size_t rowsShares = outputChannels * outputRows * outputColumns;
  std::size_t colsShares = 1;

  // OutputShareFile << rowsShares << " " << colsShares << std::endl;
  OutputShareFile << outputChannels << " " << outputRows << " " << outputColumns << std::endl;
  for (int index = 3; index < Output_Public_Shares.size(); index++) {
    OutputShareFile << Output_Public_Shares[index] << " " << Output_Private_Shares[index] << std::endl;
  }
  ...
  ReluOutputFile << outputChannels * outputRows * outputColumns << " " << 1 << std::endl;
