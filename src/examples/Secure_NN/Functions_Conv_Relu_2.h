#ifndef FUNCTIONS_CONV_RELU_H
#define FUNCTIONS_CONV_RELU_H
...
#include "GlobalVar_Conv_Relu_2.h"
...
int ShareConvert()
{
  ...
  // int partition = len / 2;
  // std::thread threadPC_0_1(PrivateCompare, std::ref(c0), std::ref(c1), std::ref(betaP), 0, partition);
  // std::thread threadPC_0_2(PrivateCompare, std::ref(c0), std::ref(c1), std::ref(betaP), partition, len);
...
int ComputeMSB() {
  ...
  // int partition = len / 2;
  // std::thread threadPC_0_1(PrivateCompare, std::ref(MSB_c0), std::ref(MSB_c1), std::ref(betaP), 0, partition);
  // std::thread threadPC_0_2(PrivateCompare, std::ref(MSB_c0), std::ref(MSB_c1), std::ref(betaP), partition, len);
