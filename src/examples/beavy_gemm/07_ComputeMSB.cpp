...
void ComputeMSB(std::vector<uint64_t>& a0_odd, std::vector<uint64_t>& a1_odd, std::vector<uint64_t>& a0_MSB_L, std::vector<uint64_t>& a1_MSB_L, std::size_t len)
{
std::cout << "ComputeMSB \n";
...
std::vector<uint64_t> r0_odd(len, 0), r1_odd(len, 0), r(len, 0);
...
std::vector<uint64_t> beta(len, 0);//At p0 and P1
...
std::vector<uint64_t> gamma0_L(0, len), gamma1_L(0, len);
std::vector<uint64_t> delta0_L(0, len), delta1_L(0, len);
std::vector<uint64_t> theta0_L(0, len), theta1_L(0, len);
//@ Party 0 
...
for(int i = 0; i < len; i++)
 {
 gamma0_L[i] = beta0_p_L[i]*(1-2*beta[i]);
 delta0_L[i] = x0_lsb[i]*(1-2*r[i] & 1);
 }
 //@Party 1
for(int i = 0; i < len; i++)
 {
 gamma1_L[i] = beta1_p_L[i]*(1-2*beta[i]) + beta[i];
 delta1_L[i] = x1_lsb[i]*(1-2*(r[i] & 1)) + (r[i] & 1);
 }
...
for(int i = 0; i < len; i++)
 {
 theta0_L[i] = gamma0_L[i]*delta0_L[i] + gamma0_L[i]*delta1_L[i];
 theta1_L[i] = gamma1_L[i]*delta1_L[i] + gamma1_L[i]*delta0_L[i];
 }
for(int i = 0; i<len; i++)
{
  a0_MSB_L[i] = gamma0_L[i] + delta0_L[i] - 2*theta0_L[i];
  a1_MSB_L[i] = gamma1_L[i] + delta1_L[i] - 2*theta1_L[i];
}
...
int main(int argc, char* argv[]) 
{
  ...
  std::vector<std::uint64_t> a_Lmiusone_0(len, 0), a_Lmiusone_1(len, 0);
  data[0] = 10;
  data[1] = LMINUS_ONE -1;
  data[2] = 0.5;
  data[3] = 0.1;
