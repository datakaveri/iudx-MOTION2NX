...
std::vector<std::uint64_t> PrivateCompare(std::vector<std::uint64_t> &bit_share_x0, std::vector<std::uint64_t> &r, std::vector<std::uint64_t> &beta, std::size_t len, int party_id = 0) 
{
...
PrivateCompare_P2(c0, c1, beta_prime, len);
for(int i = 0; i<len; i++)
{
if (beta[i] + beta_prime[i] == 1)
  ...
  // GenerateLSBSharesOverL(lsb_share_x0, lsb_share_x1, x, len);
  // std::cout << std::hex <<"LMINUS_ONE : " << LMINUS_ONE << "\n";
...
//   GenerateModuloOddShares(x0, x1, x, len);
//   for(int i = 0; i < len; i++)
//   {
//     std::cout << std::hex << " shares : " << x0[i] << " , " <<  x1[i]  << " , Clear : " << x[i] << ", Adding Shares : " << (x0[i]+x1[i]+WrapAround(x0[i], x1[i])) % LMINUS_ONE << "\n";
//   }
// for(int i = 0; i < PRIME_NUM; i++)
// {
//   std::cout << "\n";
//   for(int j = 0; j < PRIME_NUM; j++ )
//   {
//     //std::cout << addModPrime[i][j] << " , " ;
//     std::cout << subModPrime[i][j] << " , ";
//   }
  
// }
// std::uint64_t a = LMINUS_ONE;
// std::uint64_t b = 1;
// std:: cout <<  "wrap around :  " <<WrapAround(a, b) << "\n";
// std::uint64_t p = std::numeric_limits<std::uint64_t>::max();
// std::cout << std::hex << LMINUS_ONE << "\n"; 
// std::cout << std::hex << minus_one << "\n";
// std::cout << std::hex << p << "\n";
return EXIT_SUCCESS;
