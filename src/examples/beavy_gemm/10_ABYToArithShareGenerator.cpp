// At Server 0:
// ./bin/10_ABYToArithShareGenerator --my_id 0 --filepath ${BASE_DIR}/build_debwithrelinfo_gcc/3PC_Relu/X_OutputShare_0 --current-path ${BASE_DIR}/build_debwithrelinfo_gcc

// At Server 1:
// ./bin/10_ABYToArithShareGenerator --my_id 1 --filepath ${BASE_DIR}/build_debwithrelinfo_gcc/3PC_Relu/X_OutputShare_1 --current-path ${BASE_DIR}/build_debwithrelinfo_gcc
...
std::optional<Options> parse_program_options(int argc, char* argv[]) {
  ...
  // options.X_Arith_Shares = baseDirectory + "/build_debwithrelinfo_gcc/3PC_Relu/X_ArithShare_" + std::to_string(options.my_id);
...
int main(int argc, char* argv[]) {
  ...
  if (!timeDetailsFile.is_open()) {
    std::cerr << "Error: Unable to open AverageTimeDetails file." << std::endl;
  } else {
  ...
  if (!memoryDetailsFile.is_open()) {
    std::cerr << "Error: Unable to open MemoryDetails file." << std::endl;
  } else {
    memoryDetailsFile << "Execution time - " << duration.count() << "msec" << std::endl;
  }
