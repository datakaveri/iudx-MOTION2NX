...
port1_inference=3390
port0_inference=4567
...
$build_path/bin/image_provider_iudx --compute-server0-ip $cs0_ip --compute-server0-port $cs0_port_image --compute-server1-ip $cs1_ip --compute-server1-port $cs1_port_image --fractional-bits $fractional_bits --NameofImageFile X$i --filepath $image_path >> $debug_1/image_provider.txt &
pid3=$!
...
for i in "${image_ids[@]}"
do 
echo "image_ids X"$i >> MemoryDetails0
