...
# $build_path/bin/output_shares_receiver --my-id 0 --listening-port 8899 --current-path $image_provider_path --index $image_ids> $build_path/server0/debug_files/output_shares_receiver0.txt &
# pid5=$!

# $build_path/bin/output_shares_receiver --my-id 1 --listening-port 2233 --current-path $image_provider_path --index $image_ids> $build_path/server1/debug_files/output_shares_receiver1.txt &
# pid6=$!

echo "Image Provider listening for the inferencing result"

$build_path/bin/tensor_gt_relu --my-id 0 --party 0,$ip0,$port0 --party 1,$ip1,$port1 --arithmetic-protocol beavy --boolean-protocol yao --fractional-bits $fractional_bits --filepath file_config_input0 --current-path $build_path > $build_path/server0/debug_files/tensor_relu0_layer1_split.txt &
pid1=$!
...
# $build_path/bin/final_output_provider --my-id 0 --connection-port 8899 --config-input X$image_ids --current-path $build_path > $build_path/server0/debug_files/final_output_provider0_split.txt 
# pid3=$!
