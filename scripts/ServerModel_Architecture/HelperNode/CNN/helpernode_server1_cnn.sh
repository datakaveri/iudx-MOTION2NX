...
# model_config=${BASE_DIR}/config_files/example-model_helpernode_config.json
...
#       # $build_path/bin/10_ABYToArithShareGenerator --my_id 1 --filepath $build_path/server1/outputshare_1 --current-path $build_path > $debug_1/3PC_Relu_layer${layer_id}.txt &
#       # pid2=$!
#       # wait $pid2
#       # check_exit_statuses $?

#       # $build_path/bin/3PC_Relu1 --party 0,$cs0_host,$relu0_port_inference --party 1,$cs1_host,$relu1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path $build_path > $debug_1/3PC_Relu_layer${layer_id}.txt &
#       # pid3=$!
#       # wait $pid3
#       # check_exit_statuses $?
...
new_layer_types=(${layer_types[@]:1})
echo "${new_layer_types[@]}"

#######################################Matrix multiplication layer 1 ###########################################################################

  $build_path/bin/3PC_CNN_NN_1 --WB-file file_config_model1 --input-file $input_config --layers $number_of_layers --layer-types ${new_layer_types[@]} --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path $build_path > $debug_1/server1_pipeline.txt &
  pid1=$!
  wait $pid1
  check_exit_statuses $?

   $build_path/bin/tensor_argmax --my-id 1 --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --arithmetic-protocol beavy --boolean-protocol yao --fractional-bits $fractional_bits --filepath file_config_input1 --current-path $build_path > $debug_1/server1_argmax.txt &
   pid2=$!
   wait $pid2
   check_exit_statuses $?
   echo "Layer $layer_id: Argmax is done"
