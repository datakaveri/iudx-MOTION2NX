...
#       # $build_path/bin/10_ABYToArithShareGenerator --my_id 0 --filepath $build_path/server0/outputshare_0 --current-path $build_path > $debug_0/3PC_Relu_layer${layer_id}.txt &
#       # pid2=$!
#       # wait $pid2
#       # check_exit_statuses $?

#       # $build_path/bin/3PC_Relu0 --party 0,$cs0_host,$relu0_port_inference --party 1,$cs1_host,$relu1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path $build_path > $debug_0/3PC_Relu_layer${layer_id}.txt &
#       # pid3=$!
#       # wait $pid3
#       # check_exit_statuses $?     
...
####################################### Start: NN inference pipeline ###########################################################################

   start=$(date +%s)

   $build_path/bin/3PC_CNN_NN_0 --WB-file file_config_model0 --input-file $input_config --layers $number_of_layers --layer-types ${layer_types_array[@]} --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path $build_path > $debug_0/server0_pipeline.txt &
   pid1=$!
   wait $pid1
   check_exit_statuses $?

   $build_path/bin/tensor_argmax --my-id 0 --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --arithmetic-protocol beavy --boolean-protocol yao --fractional-bits $fractional_bits --filepath file_config_input0 --current-path $build_path > $debug_0/server0_argmax.txt&
   pid2=$!
   wait $pid2
   check_exit_statuses $?
   echo "Layer $layer_id: Argmax is done"
