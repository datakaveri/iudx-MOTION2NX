...
if [ ${layer_types[layer_id]} -eq 1 ] && [ $layer_id -eq 1 ];then 
   ...
   $build_path/bin/10_ABYToArithShareGenerator --my_id 1 --filepath $build_path/server1/outputshare_1 > $debug_1/3PC_Relu_layer${layer_id}.txt --current-path $build_path &
   pid2=$!
   wait $pid2
   check_exit_statuses $?
