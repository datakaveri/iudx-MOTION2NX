...
#   wait $pid1
#   check_exit_statuses $?
      # $build_path/bin/3PC_Conv_Relu_1 --WB_file file_config_model1 --input_file $input_config --layer-id $layer_id --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path ${BASE_DIR}/build_debwithrelinfo_gcc > $debug_1/server1_Conv_Relu_${layer_id}.txt  &
      # pid1=$!
      # wait $pid1
      # check_exit_statuses $?
...
#   $build_path/bin/3PC_MatMul_Relu_1 --WB_file file_config_model1 --input_file $input_config --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path $build_path --layer-id 5 --fractional-bits $fractional_bits > $debug_1/server1_MatMul{$layer_id}.txt &
#   pid2=$!
#   wait $pid2
#   check_exit_statuses $?
