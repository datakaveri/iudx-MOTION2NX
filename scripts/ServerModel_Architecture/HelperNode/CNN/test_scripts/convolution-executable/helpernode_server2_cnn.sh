...
# ####################################### Matrix multiplication layer 1 ###########################################################################
   ...
   wait $pid1
   check_exit_statuses $?
   # $build_path/bin/3PC_CNN_NN_debug_2 --layers $number_of_layers --layer-types ${layer_types_array[@]} --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path ${BASE_DIR}/build_debwithrelinfo_gcc > $debug_2/test2.txt &
   # pid1=$!
   # wait $pid1
   # check_exit_statuses $?
   #       $build_path/bin/3PC_Conv_Relu_2 --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path ${BASE_DIR}/build_debwithrelinfo_gcc > $debug_2/helpernode_layer2.txt &
   # pid1=$!
   # wait $pid1
   # check_exit_statuses $?
   # echo "Helper node layer $layer_id is done"

#   for((layer_id=4; layer_id<5; layer_id++))
#   do
#       $build_path/bin/3PC_Conv_Relu_2 --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path ${BASE_DIR}/build_debwithrelinfo_gcc > $debug_2/helpernode_layer2.txt &
#    pid2=$!
#    wait $pid2
...
#   done
   ...
   # $build_path/bin/3PC_MatMul_Relu_2 --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path $build_path > $debug_2/server2_MatMul{$layer_id}.txt &
   # pid2=$!
   # wait $pid2
   # check_exit_statuses $?
   # echo "Helper node layer $layer_id is done"

   # $build_path/bin/server2 --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path $build_path > $debug_2/helpernode_layer6.txt &
   # pid3=$!
   # wait $pid3
   # check_exit_statuses $?
   # echo "Helper node layer $layer_id is done"
