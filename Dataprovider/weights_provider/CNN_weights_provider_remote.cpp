...
class Matrix {
 private:
  uint64_t type, rows, columns, num_elements, fractional_bits, kernel, channels;
 ...
 public:
  ...
  Matrix(int ker, int chnls, int row, int col, std::string fileName, std::size_t fraction_bits,
         const Options& options) {
    type = 1;       // If CNN
    rows = row;
    columns = col;
  ...
  std::vector<int> getRowsColumns() {
    ...
    if (type == 1) {  // If CNN
      size.push_back(kernel);
      size.push_back(channels);
    }
    size.push_back(rows);
...
int main(int argc, char* argv[]) {
  ...
  std::string p1 = options->fullfilepath + "/W1.csv";
  Matrix weightW1 = Matrix(5, 1, 5, 5, p1, options->fractional_bits, *options);   // CNN Constructor
  weightW1.readMatrixCSV();
  weightW1.generateShares();

  p1 = options->fullfilepath + "/B1.csv";
  Matrix biasB1 = Matrix(5, 1, p1, options->fractional_bits, *options);
  biasB1.readMatrixCSV();
  biasB1.generateShares();

  p1 = options->fullfilepath + "/W2.csv";
  Matrix weightW2 = Matrix(100, 845, p1, options->fractional_bits, *options);
  weightW2.readMatrixCSV();
  weightW2.generateShares();

  p1 = options->fullfilepath + "/B2.csv";
  Matrix biasB2 = Matrix(100, 1, p1, options->fractional_bits, *options);
  biasB2.readMatrixCSV();
  biasB2.generateShares();

  p1 = options->fullfilepath + "/W3.csv";
  Matrix weightW3 = Matrix(10, 100, p1, options->fractional_bits, *options);
  weightW3.readMatrixCSV();
  weightW3.generateShares();

  p1 = options->fullfilepath + "/B3.csv";
  Matrix biasB3 = Matrix(10, 1, p1, options->fractional_bits, *options);
  biasB3.readMatrixCSV();
  biasB3.generateShares();
  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (auto Vector : allData) {
      ...
      int typeOfVector = Vector->getType();
      cout << typeOfVector << endl;
      auto arr1 = Vector->getRowsColumns();
      int no_dims;
      int num_elements = Vector->getNumberofElements();
      ...
      boost::system::error_code error_type;
      boost::asio::write(socket, boost::asio::buffer(&typeOfVector, sizeof(typeOfVector)), error_type);     //
      if (!error_type) {
        cout << "Not an error" << endl;
      } else {
        cout << "send failed: " << error_type.message() << endl;
      }
      
      // getting a response from the server
      boost::asio::streambuf receive_buffer_type;
      boost::asio::read_until(socket, receive_buffer_type, "\n");                                           //
      if (error_type && error_type != boost::asio::error::eof) {
        cout << "receive failed: " << error_layer.message() << endl;
      } else {
        const char* data = boost::asio::buffer_cast<const char*>(receive_buffer_type.data());
        cout << data << endl;
      }
      ...
      cout << "....Sending data...."
           << "\n";
      write_struct_vector(socket, data, num_elements);                                                 //
      cout << "Data sent"
           << "\n";
