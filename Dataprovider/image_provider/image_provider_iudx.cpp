...
int share_generation(std::ifstream& image_data, int num_elements, Shares* cs0_data, Shares* cs1_data, size_t fractional_bits) 
{
  // get input data
  ...
  if(data_size!=(num_elements+1))
    {
      std::cerr<<"Image file should have "<<num_elements<<" values. It has "<<data_size<<" values."<<std::endl;
      return EXIT_FAILURE;
    }
  ...
  try
    ...
    for (int i = 1; i < data_size; i++) 
      ...
      std::uint64_t Del = del0 + del1 + MOTION::new_fixed_point::encode<uint64_t, float>(data[i], fractional_bits);

      // For each data, creating 2 shares variables - 1 for computer server 0  and another for compute server 1
      Shares cs0, cs1;
...
int main(int argc, char* argv[]) {
  ...
  try{
      ...
      int is_error = share_generation(image_file, num_elements, cs0_data, cs1_data, options->fractional_bits);
      if(is_error)
        {
          image_file.close();
          return EXIT_FAILURE;
  ...
  for (int id = 0; id < 2; id++) 
  {
    ...
    if(establishConnection(socket,ip,port))
      ...
      {
        std::cerr<<"Connection could not established with the image share receiver in server "<<id<<std::endl;
        socket.close();
        return EXIT_FAILURE;
      }
    ...
    boost::asio::write(socket, boost::asio::buffer(&options->fractional_bits, sizeof(options->fractional_bits)),send_error);
    if (send_error) 
      {
      std::cerr << "Error: Could not send the number of fractional bits: " << send_error.message() << std::endl;
      socket.close();
      return EXIT_FAILURE;
      }
    ...
    if (read_error) 
      {
        if(read_error == boost::asio::error::eof) 
          {
          std::cerr << "Connection closed by image share receivers. Read error: " << read_error.message() << std::endl;
          } 
        else
          {
          std::cerr << "Error while receiving acknowledgment from image share receivers for fractional bits message. Read error: " << read_error.message() << std::endl;
          }
        socket.close();
        return EXIT_FAILURE;
      }
    ...
    if (send_error) 
      {
      std::cerr << "Unable to send image size (rows,columns). Error: " << send_error.message() << std::endl;
      socket.close();
      return EXIT_FAILURE;
      }
    ...
    if (read_error) 
      {
        if(read_error == boost::asio::error::eof) 
          ...
          {
          std::cerr << "Error while receiving acknowledgment from image share receivers for the image size message. Read error: " << read_error.message() << std::endl;
          }
        socket.close();
        return EXIT_FAILURE;
      }
    ...
    {       
      ...
      {
        std::cerr<<"Error while sending image shares to server "<<id<<". Error: "<<e.what()<<std::endl;
        socket.close();
        return EXIT_FAILURE;
      }
  }
